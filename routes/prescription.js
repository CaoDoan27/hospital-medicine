...
router.get('/', isAuthenticated, authorize('bac_si'), async (req, res) => {
  try {
...
// Form kê đơn mới
router.get('/tao', isAuthenticated, authorize('bac_si'), async (req, res) => {
  try {
    const [drugs] = await db.query('SELECT id, ma_bhyt, ten_thuoc, ham_luong, don_vi_tinh FROM thuoc WHERE trang_thai = 1 ORDER BY ten_thuoc');
    const [icd10] = await db.query('SELECT ma_benh, ten_benh FROM danh_muc_icd10 ORDER BY ma_benh');
...
// API: tìm bệnh nhân (dùng cho form kê đơn)
router.get('/api/benh-nhan', isAuthenticated, authorize('bac_si'), async (req, res) => {
  try {
...
// Lưu đơn thuốc mới
router.post('/luu', isAuthenticated, authorize('bac_si'), async (req, res) => {
  ...
  try {
    ...
    if (!bnId) {
      ...
      const [result] = await conn.query('INSERT INTO benh_nhan SET ?', {
    ...
    const [dotResult] = await conn.query('INSERT INTO dot_dieu_tri SET ?', {
    ...
    // Create don_thuoc
    const [dtResult] = await conn.query('INSERT INTO don_thuoc SET ?', {
    ...
    for (const it of parsedItems) {
      ...
      await conn.query('INSERT INTO chi_tiet_don_thuoc SET ?', {
...
router.get('/chi-tiet/:id', isAuthenticated, authorize('bac_si'), async (req, res) => {
  try {
    const [rows] = await db.query(`
    ...
    `, [req.params.id, req.session.user.ho_ten]);
...
// Hủy đơn thuốc (chỉ khi trang_thai='moi')
router.post('/huy/:id', isAuthenticated, authorize('bac_si'), async (req, res) => {
  try {
    const [check] = await db.query("SELECT trang_thai FROM don_thuoc WHERE id = ? AND bac_si_ke = ?", [req.params.id, req.session.user.ho_ten]);
    ...
    await db.query("UPDATE don_thuoc SET trang_thai = 'huy' WHERE id = ?", [req.params.id]);
